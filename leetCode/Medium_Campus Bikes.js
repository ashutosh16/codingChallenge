...
var assignBikes = function(workers, bikes) {
  ...
  for(let w=0; w < workers.length; w++) {
    for(let b=0; b < bikes.length; b++) {
      ...
      if(typeof distence[d] === "undefined") {
        distence[d] = [{w,b}];
      } else {
        distence[d].push({w,b});
      }
  ...
  for(let i=0; i < distence.length; i++) {
    ...
    for(let j=0; j < distence[i].length; j++) {
      ...
      const bikeId = distence[i][j].b;
      if(workersAssignment[workerId] === -1 && bikeAssignment[bikeId] === -1) {
        workersAssignment[workerId] = bikeId;
        bikeAssignment[bikeId] = workerId;
      }
    }
  }
